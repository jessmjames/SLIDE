"""
Figure 5D — Step 3: plot GB1 directed evolution (decay curve, strategy space, DE).
...
Output
------
  figures/GB1_decay.pdf
  figures/GB1_strategy_space.pdf
  figures/GB1_DE.pdf

Usage
-----
  python figures_src/figure_5D_GB1_DE/3_plot.py

Originally from ruggedness_figures_plots.ipynb cells 57, 58, 59.
"""
...
with open(os.path.join(plot_data_dir, 'GB1_strategy_selection.pkl'), 'rb') as f:
    x_vals, decay_mean, decay_rate, sweep, scipy_freq_matrix, run, scatter, line, GB1_decay_multi = pickle.load(f)
...
out_path = os.path.join(figures_dir, 'GB1_decay.pdf')
plt.savefig(out_path, dpi=dpi)
print(f'Saved -> {out_path}')


# --- Cell 58: GB1 strategy space ---
...
out_path = os.path.join(figures_dir, 'GB1_strategy_space.pdf')
plt.savefig(out_path, dpi=dpi, bbox_inches='tight')
print(f'Saved -> {out_path}')
plt.show()


# --- Cell 59: GB1 directed evolution ---
...
out_path = os.path.join(figures_dir, 'GB1_DE.pdf')
plt.savefig(out_path, dpi=dpi)
print(f'Saved -> {out_path}')
plt.show()
