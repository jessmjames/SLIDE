"""
Figure 5E — Step 2: build empirical lookup (if needed) and run TrpB directed evolution.

Uses the shared N4A20 empirical lookup (plot_data/empirical_lookup.pkl).
If that file is absent it is generated inline from N4A20 data; otherwise the
existing file is reused (so running Fig 5D first is sufficient but not required).

Pipeline (notebook cells 106–114):
  1. Ensure plot_data/empirical_lookup.pkl exists (cells 106–108).
  2. Derive thresholds/base_chances/splits for 7×7 grid (cell 109).
  3. Load TrpB decay curves, strategy sweep, and landscape array (cells 110, 34).
  4. Run empirical_strategy_selection() → predict optimal strategy from TrpB decay.
  5. Run test_strategy_empirical() — 100 reps × 2 strategies × 10 starts (cell 114).
  6. Save plot_data/TrpB_strategy_selection.pkl.

WARNING: Step 5 is GPU-intensive.
⚠️  strategy_sweep_TrpB_multistart_100_uniform_m0.025.pkl has no generation
    script on main — see 1_raw_data.py for details.

Input
-----
  SLIDE_data/N4A20_strategy_sweep.pkl      (only if empirical_lookup.pkl absent)
  SLIDE_data/N4A20_decay_curves.pkl        (only if empirical_lookup.pkl absent)
...
Output
------
  plot_data/empirical_lookup.pkl  (created if absent; shared with Figs 5D, 5F)
  plot_data/TrpB_strategy_selection.pkl
    (x_vals, decay_mean, decay_rate, sweep, scipy_freq_matrix, run,
     scatter, line, TrpB_decay_multi)

Source
------
  ruggedness_figures_data_processing.ipynb, cells 34, 106–114.
"""
...
out_trpb   = os.path.join(plot_data_dir, 'TrpB_strategy_selection.pkl')
...
# ---------------------------------------------------------------------------
# Step 1: ensure empirical lookup exists (cells 106–108)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Step 2: TrpB — load data (cells 110, 34)
# ---------------------------------------------------------------------------

if os.path.exists(out_trpb):
    print(f'Already exists: {out_trpb}')
    sys.exit(0)
...
# ---------------------------------------------------------------------------
# Step 3: strategy selection + DE (cell 114)
# ---------------------------------------------------------------------------

x_vals, decay_mean, decay_rate, sweep, scipy_freq_matrix, strategy, scatter, line = \
    empirical_strategy_selection(TrpB_decay_multi, TrpB_sweep_multi,
...
with open(out_trpb, 'wb') as f:
    pickle.dump((x_vals, decay_mean, decay_rate, sweep, scipy_freq_matrix,
                 run, scatter, line, TrpB_decay_multi), f)
print(f'Saved → {out_trpb}')
