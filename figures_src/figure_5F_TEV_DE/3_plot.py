"""
Figure 5F — Step 3: plot TEV directed evolution (decay curve, strategy space, DE).
...
Output
------
  figures/TEV_decay.pdf
  figures/TEV_strategy_space.pdf
  figures/TEV_DE.pdf

Usage
-----
  python figures_src/figure_5F_TEV_DE/3_plot.py

Originally from ruggedness_figures_plots.ipynb cells 69, 70, 71.
"""
...
with open(os.path.join(plot_data_dir, 'TEV_strategy_selection.pkl'), 'rb') as f:
    x_vals, decay_mean, decay_rate, sweep, scipy_freq_matrix, run, scatter, line, TEV_decay_multi = pickle.load(f)
...
out_path = os.path.join(figures_dir, 'TEV_decay.pdf')
plt.savefig(out_path, dpi=dpi)
print(f'Saved -> {out_path}')


# --- Cell 70: TEV strategy space ---
...
out_path = os.path.join(figures_dir, 'TEV_strategy_space.pdf')
plt.savefig(out_path, dpi=dpi, bbox_inches='tight')
print(f'Saved -> {out_path}')
plt.show()


# --- Cell 71: TEV directed evolution ---
...
out_path = os.path.join(figures_dir, 'TEV_DE.pdf')
plt.savefig(out_path, dpi=dpi)
print(f'Saved -> {out_path}')
plt.show()
