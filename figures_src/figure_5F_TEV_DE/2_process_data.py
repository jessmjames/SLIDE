"""
Figure 5F — Step 2: build empirical lookup (if needed) and run TEV directed evolution.
...
Pipeline (notebook cells 106–115):
  ...
  3. Load TEV decay curves, strategy sweep, and landscape array (cells 110, 34).
  4. Run empirical_strategy_selection() → predict optimal strategy from TEV decay.
  5. Run test_strategy_empirical() — 100 reps × 2 strategies × 10 starts (cell 115).
  6. Save plot_data/TEV_strategy_selection.pkl.

WARNING: Step 5 is GPU-intensive.
⚠️  strategy_sweep_TEV_multistart_100_uniform_m0.025.pkl has no generation
...
Output
------
  plot_data/empirical_lookup.pkl  (created if absent; shared with Figs 5D, 5E)
  plot_data/TEV_strategy_selection.pkl
    (x_vals, decay_mean, decay_rate, sweep, scipy_freq_matrix, run,
     scatter, line, TEV_decay_multi)

Source
------
  ruggedness_figures_data_processing.ipynb, cells 34, 106–115.
"""
...
out_tev    = os.path.join(plot_data_dir, 'TEV_strategy_selection.pkl')
...
# ---------------------------------------------------------------------------
# Step 2: TEV — load data (cells 110, 34)
# ---------------------------------------------------------------------------

if os.path.exists(out_tev):
    print(f'Already exists: {out_tev}')
    sys.exit(0)
...
# ---------------------------------------------------------------------------
# Step 3: strategy selection + DE (cell 115)
# ---------------------------------------------------------------------------

x_vals, decay_mean, decay_rate, sweep, scipy_freq_matrix, strategy, scatter, line = \
    empirical_strategy_selection(TEV_decay_multi, TEV_sweep_multi,
...
with open(out_tev, 'wb') as f:
    pickle.dump((x_vals, decay_mean, decay_rate, sweep, scipy_freq_matrix,
                 run, scatter, line, TEV_decay_multi), f)
print(f'Saved → {out_tev}')
