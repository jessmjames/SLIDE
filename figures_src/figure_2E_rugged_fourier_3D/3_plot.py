"""
Figure 2E — Step 3: plot rugged landscape Fourier-space representation.

Input
-----
  No pkl files. All data computed inline.
  Projects bump_func(x,y) = -sin(x*pi/2 + y*pi/2)*cos(y*pi/4) onto the real
  orthonormal Fourier basis for N=4 and plots the 4×4 coefficient surface as a
  3D figure.

Output
------
  figures/rugged_fourier_3D.pdf

Usage
-----
  python figures_src/figure_2E_rugged_fourier_3D/3_plot.py
...
# ---- Cell 91: plot rugged Fourier space ----
import matplotlib.pyplot as plt
from matplotlib import cm
from matplotlib.colors import Normalize
...
out_path = os.path.join(figures_dir, 'rugged_fourier_3D.pdf')
