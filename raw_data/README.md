...
| `nk_strategy_grid_M50_raw_data.pkl` | **Fig S5** | same, M=50 | as above but **50 landscapes**, M=50 | ~23 min |
...
| `nk_strategy_N3_A20_raw_data.pkl` | Fig 5 (ParD3) | NK strategy lookup, N=3 A=20 | 5×5 grid | ~3 min |
| `empirical_decay_{GB1,TrpB,TEV,ParD3}_uniform_raw_data.pkl` | Fig 5 D–G | empirical fitness-decay curves | ≤10k starts, uniform mutation | ~1 min each |
| `empirical_strategy_{GB1,TrpB,TEV,ParD3}_uniform_raw_data.pkl` | Fig 5 D–G | empirical strategy sweep | 7×7 (5×5 ParD3) | ~0.8 min each |
| `empirical_strategy_traj_{GB1,TrpB,TEV,ParD3}_raw_data.pkl` | **Fig 5 D–G** | **best-variant trajectory of every (split,base) cell** — heatmap = gen-25 slice, DE lines = selected cells (ONE dataset) | 100 starts × 300 reps, 150 gens, pop 1200 (60 ParD3), mut 0.1/N, best-variant max | ~varies |
