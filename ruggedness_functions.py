...
def get_array_from_fun(func, shape):
    to_see = np.stack(np.indices(shape), axis=-1).reshape(
        jnp.prod(jnp.array(shape)), len(shape)
    )
    values = func(to_see)
...
def generate_range_cube(shape):
    ...
    for i, d_size in enumerate(shape):
        base_cube = base_cube.at[
            tuple(
                [
                    slice(None) if j != i else jnp.arange(1, d_size)
...
def find_acc_path_length(landscape, starting_point):
    ...
    for step in range(N):
        for i, d_size in enumerate(shape):
            move_from_slice = tuple(
                [slice(None) if j != i else slice(None, 1) for j in range(N)]
            )
            move_to_slice = tuple(
                [slice(None) if j != i else slice(1, None) for j in range(N)]
            )
            path_accesable = (rotated_landsape <= rotated_landsape[move_from_slice]) * (
                range_cube == (step + 1)
            )
            new_paths = (
                jnp.zeros_like(num_paths)
                .at[move_to_slice]
                .set(num_paths[move_from_slice])
                * path_accesable
            )
...
def find_distance_to_closest_max(landscape_arrays):
    return find_distance_to_set(find_local_max(landscape_arrays)).mean() / len(
        landscape_arrays.shape
    )
...
def get_landscape_spectrum(landscape, norm=False, remove_constant=True, on_gpu=False):
    ...
    if norm:
        collapsed_spectrum = (
            collapsed_spectrum / (collapsed_spectrum * collapsed_spectrum).sum() ** 0.5
        )
...
def get_exp_matrix(
    ...
    if fix_b0:
        return np.array(
            [
                [np.exp(-mut / (N * (A - 1)) * l * factor) - 1 for l in eigenvalues]
        ...
        return np.array(
            [
                [np.exp(-mut / (N * (A - 1)) * l * factor) for l in eigenvalues]
...
def get_fourier_coeffs(
    ...
    """
    exponentials = get_exp_matrix(
        N=N, A=A, mutations=mutations, is_squared=is_squared, fix_b0=fix_b0
    )
    if fix_b0:
        mean_fitness_0 = mean_fitness[0]
        mean_fitness = mean_fitness[1:] - mean_fitness_0
    if method == "ls":
        fourier_coeffs, residuals, rank, s = np.linalg.lstsq(
            exponentials, mean_fitness, rcond=None
        )
    ...
    if fix_b0:
        ...
        exponentials = get_exp_matrix(
            N=N, A=A, mutations=mutations, is_squared=is_squared, fix_b0=False
        )
...
def get_fourier_decay(
    fourier_coeffs: np.array, A: int, N: int, is_squared: bool = False
) -> float:
