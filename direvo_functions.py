...
def run_directed_evolution(
    ...
    def single_iteration(
        ...
        population_fitness = fitness_function(
            population
        )  # Calculate population fitness values.
        fitness_noise = (
            population_fitness * jr.normal(r1, population_fitness.shape) * fitness_noise
        )  # Add noise to fitness readings.
        ...
        extra_info = {
            key: func(**current_info_dict) for key, func in extra_function_dict.items()
        }
    ...
    def multiple_iterations(population, local_rng):
        ...
        new_pop, extra_info = single_iteration(
            local_rng,
            population,
            selection_function,
            mutation_function,
...
def build_NK_landscape_function(rng, N, K, fitness_distribution=jr.normal):
    ...
    def permutate_rows(rng, i):
        ...
        return jnp.insert(perm_row, i, 1)
...
def build_selection_function(selection_function_shape, params):
    ...
    def selection_function(rng, fitnesses, state=0):
        ...
        selected = jnp.ones((psize,)) * jr.bernoulli(
            rng, p=selection_prob, shape=(psize,)
        )
...
def base_chance_threshold_fixed_prop(base_chance_range, proportion, num_samples=10):
    ...
    base_chance_range_relevant = [
        base_chance_range[0],
        min(proportion, base_chance_range[1]),
    ]
