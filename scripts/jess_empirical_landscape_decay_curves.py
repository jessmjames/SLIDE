...
def generate_decay_curve(ld, m,p=2500):
    ...
    results = []
    # If too much memory used, changed 500 to something smallers, 20 to something bigger to copensate.
    if ld.shape == (20,20,20,20):
        all_starts = uniform_start_locs(ld).reshape(20,500,4)
    else:
        all_starts = uniform_start_locs(ld).reshape(20,500,3)
