...
def generate_decay_curve_old(ld, m, p=2500, batch_size=1000):
    """
...
def generate_decay_curve(ld, m, p=2500, batch_size=100):
    ld = jnp.array(ld)
    all_starts = all_start_locs(ld)  # shape (total_starts, ndim)
    total_starts = all_starts.shape[0]
    ndim = all_starts.shape[1]
...
for popsize in np.linspace(25,2500,10)[::-1]:
...
with open(os.path.join(slide_data_dir, f"decay_curves_GB1_m0.1_multistart_all_starts_multi_popsize2.pkl"), "wb") as f:
    pickle.dump(np.array(GB1_results[::-1]), f)

with open(os.path.join(slide_data_dir, f"decay_curves_TrpB_m0.1_multistart_all_starts_multi_popsize2.pkl"), "wb") as f:
    pickle.dump(np.array(TrpB_results[::-1]), f)

with open(os.path.join(slide_data_dir, f"decay_curves_TEV_m0.1_multistart_all_starts_multi_popsize2.pkl"), "wb") as f:
    pickle.dump(np.array(TEV_results[::-1]), f)

with open(os.path.join(slide_data_dir, f"decay_curves_ParD3_m0.1_multistart_all_starts_multi_popsize2.pkl"), "wb") as f:
