...
def uniform_start_locs(num=10000):
    flat_indexes = np.round(np.linspace(0, 159999, num)).astype(int)
    indexes = np.array([np.unravel_index(i, (20,20,20,20)) for i in flat_indexes])
    return indexes
...
all_starts = all_starts.reshape(10,10,4)
...
with open(os.path.join(slide_data_dir, "N4A20_decay_curves.pkl"), "wb") as f:
