...
def get_single_sweep_empirical(
    base_chances, thresholds, splits, popsize, num_alleles=2, num_reps=10
):
    ...
    def to_return(rng, landscape_arr, init_pop=None):
        ...
        results = jnp.moveaxis(
            results, 0, -1
        )  # Move the split dimension to the last axis
        return results

    return to_return


def get_multi_sweep_NK(
    base_chances,
    thresholds,
    splits,
    popsize,
...
def shuffle_with_mask(rng, values, mask):
    ...
    shuffled_values = flat_values.at[idx_true].set(
        jax.random.permutation(rng, flat_values[idx_true])
    )
...
def spectral_shuffle(rng, landscape, flip_dirs=True):
    ...
    if flip_dirs:
        ...
        random_flip = random_flip.at[jnp.where(range_cube == 0)].set(
            1
        )  # Ensure the constant term is not flipped
...
m = 0.1
p = 60
sweepy_NK = get_multi_sweep_NK(
    base_chances, thresholds, splits, p, num_alleles=20, num_reps=10, num_landscapes=125
)
results = [
    [sweepy_NK(ii, 3, i).mean(axis=0) for i in [1, 2, 3]]
    for ii in jr.split(jr.PRNGKey(42), 10)
]

with open(os.path.join(slide_data_dir, "N3A20_strategy_sweep.pkl"), "wb") as f:
