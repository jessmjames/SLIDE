...
def generate_decay_curve(ld, m,p=2500):
    ...
    if ld.shape == (20,20,20,20):
        all_starts = all_start_locs(ld).reshape(1600,100,4)
    else:
        all_starts = all_start_locs(ld).reshape(80,100,3)
...
with open(os.path.join(slide_data_dir, f"decay_curves_gb1_m0.1_all_starts.pkl"), "wb") as f:
...
m = 0.1
results = generate_decay_curve(ld = ld, m = m/4)
with open(os.path.join(slide_data_dir, f"decay_curves_trpb_m0.1_all_starts.pkl"), "wb") as f:
    pickle.dump(np.array(results), f)
...
m = 0.1
results = generate_decay_curve(ld = ld, m = m/4)
with open(os.path.join(slide_data_dir, f"decay_curves_tev_m0.1_all_starts.pkl"), "wb") as f:
    pickle.dump(np.array(results), f)
...
with open(os.path.join(slide_data_dir, f"decay_curves_pard3_m0.1_all_starts.pkl"), "wb") as f:
    pickle.dump(np.array(results), f)