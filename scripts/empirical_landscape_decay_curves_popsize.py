...
def get_WT_starts(arr, n_points=100, percentile=99):
    flat = arr.ravel()
...
def generate_decay_curve(ld, m,p=2500, steps=25):
...
for popsize in np.linspace(25,2500,10):
...
with open(os.path.join(slide_data_dir, f"decay_curves_GB1_m0.1_multistart_10000_uniform_multi_popsize.pkl"), "wb") as f:
    pickle.dump(np.array(GB1_results), f)

with open(os.path.join(slide_data_dir, f"decay_curves_TrpB_m0.1_multistart_10000_uniform_multi_popsize.pkl"), "wb") as f:
    pickle.dump(np.array(TrpB_results), f)

with open(os.path.join(slide_data_dir, f"decay_curves_TEV_m0.1_multistart_10000_uniform_multi_popsize.pkl"), "wb") as f:
    pickle.dump(np.array(TEV_results), f)

with open(os.path.join(slide_data_dir, f"decay_curves_ParD3_m0.1_multistart_10000_uniform_multi_popsize.pkl"), "wb") as f:
    pickle.dump(np.array(ParD3_results), f)