...
def generate_decay_curve_old(ld, m, p=4000, batch_size=1000):
    """
    ...
    Output shape: (-1, 100, 10, 40) i.e. (n_chunks, 100, num_reps, num_steps).
    This matches the shape produced by the original script and is compatible
    with downstream notebook processing (cell 52: .mean(axis=2).reshape(-1,40)).
    """
    ld = jnp.array(ld)
    ...
    def run_from_start(start):
        ...
        rep_results = jax.vmap(
            lambda r: run_directed_evolution(
                ...
                fitness_function=fitness_function, num_steps=40
            )[1]
        )(rng_seeds)
        return rep_results['fitness'].mean(axis=-1)   # (num_reps, num_steps) = (10, 40)
    ...
    for i, starts in enumerate(tqdm.tqdm(batched_starts)):
        batch_result = vmapped_run(jnp.array(starts))     # (batch_size, 10, 40)
        if pad and i == len(batched_starts) - 1:
            batch_result = batch_result[:batch_size - pad]
        results.append(batch_result)

    # Concatenate → (total_starts, 10, 40), then reshape to (-1, 100, 10, 40)
    # to match the original script's output shape for downstream compatibility.
    combined = np.array(jnp.concatenate(results, axis=0))  # (total_starts, 10, 40)
    return combined.reshape(-1, 100, 10, 40)

def generate_decay_curve(ld, m, p=4000, batch_size=100):
    ld = jnp.array(ld)
    all_starts = all_start_locs(ld)  # shape (total_starts, ndim)
    total_starts = all_starts.shape[0]
    ndim = all_starts.shape[1]
    ...
    def run_from_start(start):
        ...
        return rep_results['fitness'].mean(axis=-1)   # (10, 40)
...
print(np.linspace(40,2500,10))

for popsize in np.linspace(40,2500,10)[::-1]:
...
with open(os.path.join(slide_data_dir, f"decay_curves_ParD3_m0.1_multistart_all_starts_multi_popsize_more_steps.pkl"), "wb") as f:
