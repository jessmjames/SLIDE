...
⚠️ Provenance uncertain. The closest code is cells 83–89 of
`ruggedness_figures_data_processing_IK.ipynb` (working tree / main), which loads
`N4A20_strategy_sweep_{25,50,75,100}_gens.pkl` from `SLIDE_data/` and plots
optimal split and base chance over generations — but **no `savefig` calls exist**.
The paper figure was likely generated by running those cells interactively and
saving manually, or locally outside of git. Needs a proper `savefig` adding as
part of the rationalisation effort.
