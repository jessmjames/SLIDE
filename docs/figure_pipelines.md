...
**Source:** `ruggedness_figures_data_processing_IK.ipynb`, cells 83–89 (working tree / main)

Loads `N4A20_strategy_sweep_{25,50,75,100}_gens.pkl` from `SLIDE_data/`, computes
optimal split and base chance over generations for K=0,1,2, and plots the result.
Full plotting code is present — **just missing `savefig` calls**. The paper figure
was likely generated by running these cells interactively. Adding `savefig` is a
priority for the rationalisation effort.
