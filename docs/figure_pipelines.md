...
| S4 | `figures/strategy_prediction_25gens.pdf`, `_50gens.pdf`, `_100gens.pdf` | `ruggedness_figures_data_processing_IK.ipynb` cells 83–89 | main | ⚠️ Plotting code present, no `savefig` — needs one line adding |
...
⚠️ **Source disputed — awaiting confirmation from Jess.**

**Jess says:** branch main, source `ruggedness_figures_data_processing_IK.ipynb`,
outputs `strategy_prediction_{25,50,100}gens.pdf`.

**What we found on main in that notebook:**
- Cells 83–89: 5×3 heatmap grid + line plots of optimal split/base chance vs gens — no `savefig`
- Cells 118–122 ("Optimal strategies over increasing generations"): dual-axis line plots
  of optimal split + base chance vs generations [5,25,50,75,100] — no `savefig`
- No cell with title "Optimal DE strategies (M=X)" in any notebook on any branch

**Current `figures_src/figure_S4_strategy_gens/3_plot.py`** outputs per-generation
heatmaps (one 1×3 PDF for each of g=25,50,100) — this was a best-guess extraction
but is likely wrong. Do not use until source confirmed.

The title "Optimal DE strategies (M=20)" seen in the paper may be runtime-generated
(e.g. `f'Optimal DE strategies (M={M})'`) from code that was never committed.
